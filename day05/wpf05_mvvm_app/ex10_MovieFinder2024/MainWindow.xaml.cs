...
namespace ex10_MovieFinder2024
{
    ...
    {
        private bool isFavourite = false; // 즐겨찾기인지 API로 검색한건지 false => openAPI, true =>즐겨찾기 보기
        ...
        private async void SearchMovie(string movieName)
        {
            ...
            var jsonArray = results as JArray; // result가 json배열기이 때문에 JArray는 List와 동일해서 foreach 사용가능
        ...
        private async void BtnAddFavorite_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    await this.ShowMessageAsync("즐겨찾기", "즐겨찾기 저장완료");
                }
            }
            catch (Exception ex)
            {
        ...
        private async void BtnViewFaavorite_Click(object sender, RoutedEventArgs e)
        {
            ...
            this.DataContext = null; // 데이터그릳즈에 보낸데이터를 모두 삭제
            TxtMovieName.Text = string.Empty;

            List<MovieItem> FavMovieItem = new List<MovieItem>();
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Helpers.Common.CONNSTRING))
                {
                    ...
                    foreach (DataRow row in dSet.Tables["MovieItem"].Rows) 
                    {
                        ...
                        FavMovieItem.Add(movieItem);

                    }
                    this.DataContext = FavMovieItem;
                    isFavourite = true;
                    StsResult.Content = $"즐겨찾기 {FavMovieItem.Count}건 조회완료";
        ...
        {
            await this.ShowMessageAsync("삭제", "삭제완료");
