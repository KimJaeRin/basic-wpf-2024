...
namespace ex07_EmployeeMngApp.ViewModels
{
    ...
    {
        private IDialogCoordinator _dialogCoordinator; // 메트로방식 다이얼로그 디자인적용 객체
        // 멤버변수
        private int id;
        ...
        public async void SaveEmployee() 
        {
           ...
           using (SqlConnection conn = new SqlConnection(Helpers.Common.CONNSTRING))
           {
                ...
                if (result > 0)
                {
                    ...
                    await this._dialogCoordinator.ShowMessageAsync(this, "저장성공", "");
                }
                else
                {
                   // MessageBox.Show("저장실패");
                    await this._dialogCoordinator.ShowMessageAsync(this, "저장실패", "");
        ...
        public async void DelEmployee()
        {
            ...
            if (ID == 0)
            {
                ...
                await this._dialogCoordinator.ShowMessageAsync(this, "삭제불가","");
                return;
            }

            var val = await this._dialogCoordinator.ShowMessageAsync(this, "삭제하시겠습니까?","", MessageDialogStyle.AffirmativeAndNegative);
            ...
            using (SqlConnection conn = new SqlConnection(Helpers.Common.CONNSTRING))
            {
                ...
                if (res >= 0) 
                {
                   // MessageBox.Show("삭제성공");
                    await this._dialogCoordinator.ShowMessageAsync(this, "삭제성공", "");
                }
                else
                {
                  //  MessageBox.Show("삭제실패");
                    await this._dialogCoordinator.ShowMessageAsync(this, "삭제실패", "");
